...
def test_planner_turn_executes_provider_rich_read_only_tools(
    ...
    assert geometry_output["place_markers"][0]["description"] == "Where the journey starts."
...
def test_planner_turn_matches_notebook_focus_synonyms_and_clarifies_ambiguity(
    ...
    hotel_focus_call = next(
        item for item in hotel_reply["tool_calls"] if item["tool_name"] == "set_notebook_focus"
    )
    ...
    flight_focus_call = next(
        item for item in flight_reply["tool_calls"] if item["tool_name"] == "set_notebook_focus"
    )
    ...
    assert all(item["tool_name"] != "set_notebook_focus" for item in ambiguous_reply["tool_calls"])
    assert "Which planning area did you mean" in ambiguous_reply["content"]
    question_blocks = [
        block for block in ambiguous_reply["structured_blocks"] if block["kind"] == "question"
    ]
...
def test_session_resume_message_triggers_read_notebook_context(
    ...
    context_call = next(
        item for item in reply["tool_calls"] if item["tool_name"] == "read_notebook_context"
    )
...
def test_read_notebook_context_tool_bounds_items_per_category(
    ...
    assert any(tool["tool_name"] == "read_notebook_context" for tool in list_planner_tools())
