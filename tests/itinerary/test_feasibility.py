...
def test_public_move_cost_builder_keeps_two_tuple_contract() -> None:
    estimates, move_costs = build_move_cost_summaries(
...
def test_overnight_checkin_window_accepts_late_evening_arrival() -> None:
    ...
    payload["transport_options"][0]["timing_summary"]["arrival_local"] = "2026-04-10T23:30:00+09:00"
    ...
    assert "late_arrival_checkin_conflict" not in assessment.blocking_reasons
    assert all(
        conflict.code != "late_arrival_checkin_conflict" for conflict in assessment.timing_conflicts
    )
...
def test_overnight_checkin_window_blocks_next_day_post_cutoff_arrival() -> None:
    ...
    payload["transport_options"][0]["timing_summary"]["arrival_local"] = "2026-04-11T01:30:00+09:00"
    payload["lodging_options"][0]["booking_terms"]["checkin_window"] = "15:00-01:00"
