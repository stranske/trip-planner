...
from trip_planner.options import ActivityOption, Destination, LodgingOption, TransportOption
...
def _load_scenario(name: str) -> dict:
    ...
    return {
        ...
        "destinations": [_load_entry(item, Destination.from_dict) for item in payload["destinations"]],
        "lodging_options": [
...
def test_leisure_candidate_generation_preserves_explanations_and_freshness_exclusions() -> None:
    ...
    assert candidate_set.exclusions[0].reason_code == "stale_source"
...
def test_lodging_narrowing_keeps_available_options_and_excludes_sold_out_inventory() -> None:
    ...
    assert candidate_set.exclusions[0].option_id == "lodg-kyoto-machiya-loft"
...
def test_business_candidate_generation_filters_policy_violations_and_marks_policy_ready() -> None:
    ...
    assert candidate_set.exclusions[0].option_id == "transport-car-highland-loop"
