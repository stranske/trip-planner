...
def test_scenic_discovery_profile_keeps_scenic_bundle_as_best_penalized_option() -> (
    None
):
    engine = LeisureRankingEngine()
    ranked = engine.rank_candidate_set(
    ...
    assert (
        ranked.results[1].score_breakdown.bonuses[0].reason_code == "transit_is_feature"
    )
...
def test_ranked_results_emit_breakdowns_and_explanation_records() -> None:
    ...
    assert any(
        record.record_type == "promotion"
        for record in scenic_result.explanation_records
    )
    assert target_option is not None
...
def test_identical_candidates_reorder_between_depth_and_discovery_profiles() -> None:
    ...
    assert (
        _result_option_id(discovery_ranked.results[1])
...
def test_identical_profile_reorders_when_objectives_change() -> None:
    ...
    assert (
        _result_option_id(scenic_ranked.results[0]) == "candidate:bundle:urban-culture"
    )
    assert (
        _result_option_id(scenic_ranked.results[1])
        == "candidate:bundle:scenic-wanderer"
    )
