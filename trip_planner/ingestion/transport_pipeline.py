"""Transport ingestion scaffolding from raw snapshots to normalized options."""
...
def ingest_transport_snapshot(
    ...
    *,
    ...
    if snapshot.entity_scope != "transport":
        raise ValueError("snapshot.entity_scope must be transport")
    if snapshot.option_kind not in {"flight", "rail", "car"}:
        raise ValueError("snapshot.option_kind must be a transport option kind")
        ...
        if not records:
            warnings.append(
                IngestionWarning(
                    ...
                    message="Dedup decision did not map to any raw transport records.",
                )
            )
            continue
    ...
    summary = IngestionSummary(
        ...
        emitted_options=len(transport_options),
        skipped_records=max(0, len(snapshot.records) - len(transport_options)),
        degraded_options=sum(1 for option in transport_options if option.notes),
...
def _transport_option_from_records(
    ...
    payload["source_refs"] = [
        build_provenance_reference(
            ...
            summary=f"Normalized transport option sourced from {record.provider_entity_id}.",
        ).to_dict()
        for record in records
    ]
