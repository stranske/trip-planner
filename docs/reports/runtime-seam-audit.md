...
| `trip_planner/app/services/planner_tools.py:479` and `trip_planner/app/services/planner_tools.py:534` (`read_map_provider_status`, `read_route_geometry`) | already-correct | For missing route scenarios, both tools return explicit `not_available` state instead of fabricated provider or geometry data. `tests/app/test_planner_routes.py:785` asserts `read_map_provider_status.status == "not_available"` with `route_state == "missing"` and `read_route_geometry.status == "not_available"` with empty geometry. | None; this is the correct no-data behavior. |
...
## `not_available` Coverage Check (`planner_tools.py`)

Current `rg '"not_available"' trip_planner/app/services/planner_tools.py` matches are all mapped:

- `read_source_summary` -> `trip_planner/app/services/planner_tools.py:392` (`already-correct`)
- `read_map_provider_status` -> `trip_planner/app/services/planner_tools.py:479` (`already-correct`)
- `read_route_geometry` -> `trip_planner/app/services/planner_tools.py:534` (`already-correct`)
- `refresh_route_comparison` -> `trip_planner/app/services/planner_tools.py:601` (`already-correct`)
