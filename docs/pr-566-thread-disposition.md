...
- Local verification note: `git show --format=fuller --no-patch 88a850b8` confirms the merged PR #581 title was `Follow-up fixes for audit gaps on PRs #566 and #571 (#581)`, which references PR #566. The GitHub PR description/body still requires a human check in GitHub because it is not stored in the local checkout.
