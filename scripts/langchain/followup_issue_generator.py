...
def extract_verification_data(comment_body: str) -> VerificationData:
    ...
    if single_verdict and not data.provider_verdicts:
        ...
        confidence_match = re.search(
            r"Verdict:.*?@?\s*(\d+)%", comment_body, re.IGNORECASE
        )
...
def _get_llm_client(reasoning: bool = False) -> tuple[Any, str] | None:
    ...
    if os.environ.get("GITHUB_TOKEN"):
        print(
            f"Using GitHub Models with model: {github_default_model}", file=sys.stderr
        )
        return (
            ChatOpenAI(
    ...
    print(
        "Warning: No LLM API keys found (OPENAI_API_KEY or GITHUB_TOKEN)",
        file=sys.stderr,
    )
    return None
