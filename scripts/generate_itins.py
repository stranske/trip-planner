...
def score_segment(seg, CS):
    base = 0.35*seg["natural"] + 0.15*seg["cultural"] + 0.25*seg["global_significance"] + 0.15*seg["experience_bundle"]
...
def main():
    ...
    for seg in segments:
        seg["score"] = round(score_segment(seg, CS), 2)

    # Placeholder: pick top 10 segments for Compact, top 16 for Extended
